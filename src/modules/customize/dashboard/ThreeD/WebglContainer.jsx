...
import { GetFabApi } from '../../../../ApiService/ApiUrls';
import Preloadtexture from '../../../../assests/images/preloadtexture.jpg';
import RefreshIcon from '../../../../assests/images/refresh.png';
...
class WebglContainer extends React.Component{
    constructor(props){
        ...
        this.ContrastFabricChange = this.ContrastFabricChange.bind(this);
        this.DefaultFabric = this.DefaultFabric.bind(this);
    }
    ...
    componentDidMount(){
        ...
        this.light4 = new THREE.AmbientLight( 0xffffff, 0.5 ); 
        ...
        // for (var i = 0; i<this.props.allFabs.length; i++) {
        //     this.mapOutside[i] = new THREE.TextureLoader().load( this.BuffertoImage(this.props.allFabs[i].fabimage.data) );
        //     this.mapOutside[i].wrapS = THREE.RepeatWrapping;
        //     this.mapOutside[i].wrapT = THREE.RepeatWrapping;
        //     this.mapOutside[i].repeat.set( this.props.allFabs[i].tilex, this.props.allFabs[i].tiley );
        ...
        // this.mapOutside[0] = new THREE.TextureLoader().load( Preloadtexture );
        // this.mapOutside[0].wrapS = THREE.RepeatWrapping;
        // this.mapOutside[0].wrapT = THREE.RepeatWrapping;
        // this.mapOutside[0].repeat.set( 30,30 );
        // this.mapOutside[0].name = 'map'+1;
    
        setTimeout(() => {
            this.buildObject();        
        }, 1000);
        
        this.renderer = new THREE.WebGLRenderer({canvas:this.mount, antialias: true, alpha: true});
        this.renderer.setPixelRatio(window.devicePixelRatio);
        var container = document.getElementById('webglparent');
        this.renderer.setSize(370, 700);
    ...
    ChangeStyleName(nameClicked) {
        this.currentSelection = nameClicked;
    }
    
    async GetFabImage(id){
        let _this = this;
        let response = await GetApi.getParamCall(GetFabApi+id)
                        .then(async function(res){
                            let resp = res.data;
                            _this.mapOutside[0] = new THREE.TextureLoader().load( _this.BuffertoImage(resp.fabimage.data) );
                            _this.mapOutside[0].wrapS = THREE.RepeatWrapping;
                            _this.mapOutside[0].wrapT = THREE.RepeatWrapping;
                            _this.mapOutside[0].repeat.set( resp.tilex, resp.tiley );
                            _this.mapOutside[0].name = 'map'+resp._id;
                            _this.ChangeFabric()   
                        })
                        .catch(function(err) {
                            ////console.log(err)
    ...
    async FabricChange(fab, fabImage){
        //this.GetFabImage(id);
        // if (this.currentSelection !== "Shirt" || this.currentSelection !== "Dress"){
        //     this.currentSelection = this.tempSelection;
        // }
        this.prevFabric.fab = fab;
        this.prevFabric.fabImage = fabImage;
        this.mapOutside[0] = new THREE.TextureLoader().load( this.BuffertoImage(fabImage) );
        this.mapOutside[0].wrapS = THREE.RepeatWrapping;
        this.mapOutside[0].wrapT = THREE.RepeatWrapping;
        this.mapOutside[0].repeat.set( fab.tilex, fab.tiley );
        this.mapOutside[0].name = 'map'+fab._id;
        this.ChangeFabric();   
    }

    DefaultFabric(parentName){
        this.currentSelection = parentName;
        this.mapOutside[0] = new THREE.TextureLoader().load( this.BuffertoImage(this.prevFabric.fabImage) );
        this.mapOutside[0].wrapS = THREE.RepeatWrapping;
        this.mapOutside[0].wrapT = THREE.RepeatWrapping;
        this.mapOutside[0].repeat.set( this.prevFabric.fab.tilex, this.prevFabric.fab.tiley );
        this.mapOutside[0].name = 'map'+this.prevFabric.fab._id;
        this.ChangeFabric();   
    }

    ContrastFabricChange(fab, parentName){
        //this.currentSelection = parentName;
        let newMap = new THREE.TextureLoader().load( fab.src );
        newMap.wrapS = THREE.RepeatWrapping;
        newMap.wrapT = THREE.RepeatWrapping;
        newMap.repeat.set( fab.tilex, fab.tiley );
        newMap.name = 'map'+fab._id;
        // this.mapOutside[0] = new THREE.TextureLoader().load( fab.src );
        // this.mapOutside[0].wrapS = THREE.RepeatWrapping;
        // this.mapOutside[0].wrapT = THREE.RepeatWrapping;
        // this.mapOutside[0].repeat.set( fab.tilex, fab.tiley );
        // this.mapOutside[0].name = 'map'+fab._id;
        ...
        this.model.traverse( function( node ) {
            if ( node.name === parentName) {
                ...
                node.traverse( function( subNode ) {
                    if ( subNode instanceof THREE.Mesh ) {
                        var newMaterial = subNode.material.clone();
                        newMaterial.map = newMap;
                        subNode.material = newMaterial;
                        subNode.material.needsUpdate = true;
                    }
                });
            }
        ...
        this.model.traverse( function( node ) {
            if ( node.name === _this.currentSelection) {
                ...
                node.traverse( function( subNode ) {
                    if ( subNode instanceof THREE.Mesh ) {
                        console.log(subNode)
                        subNode.material.map = _this.mapOutside[0];
                        subNode.material.needsUpdate = true;
                    }
                });
            }
    ...
    onWindowResize() {
        ...
        this.renderer.setSize( container.offsetWidth , container.offsetHeight > 800 ? 800 : container.offsetHeight );
    ...
    MaleVisible(jsonType, json){
        //this.buildObject()
        //this.getJson()
        this.visibility['Dress'] = false;
        this.visibility['Churidar'] = false;
        this.visibility["Female"] = false;
    ...
    FemaleVisible(jsonType, json){
        if (!this.state.isFemaleLoad){
            this.buildObject(json, true);    
            this.setState({
                isFemaleLoad: true
            })
        }
        //this.getJson(jsonType);
        this.visibility['Dress'] = true;
        this.visibility['Churidar'] = true;
        this.visibility["Female"] = true;
        this.visibility["Shirt"] = false;
        this.visibility["Shoe_Body"] = false;
        this.visibility["Trouser"] = false;
        // this.visibility["Waistcoat"] = false;
        // this.visibility["Jacket"] = false;
        // this.visibility["Waistcoat_Shirt"] = false;
        // this.visibility["Jacket_Waistcoat"] = false;
        this.currentSelection = "Dress";
        this.tempSelection = "Dress";
        const entries = Object.entries(this.visibility);
    ...
    createMesh(name,path,i,map,obj,boss,status) {
        ...
        this.mtlLoader.load( path.slice(0,-4)+".mtl", function( materials ) {
            ...
            _this.objLoader.load(path, function ( object ) {
                ...
                object.scale.set(0.050,0.048,0.048);
                ...
                if ( object.children.length > 1 ) {
                    ...
                    for (var i = 0; i<object.children.length; i++) {
                        ...
                        object.children[i].material.side = THREE.DoubleSide;
                        if (name !== "body"){
                            object.children[i].material.color.set(0xffffff);
                        }
    ...
    render(){
        return(
            <>  
                <div className='block' id='webglparent'>
                {/* <button title="Contact Sale" 
                            className="fixed z-90 right-8 bg-blue-600 w-20 h-20 rounded-full drop-shadow-lg flex justify-center items-center text-white text-4xl hover:bg-blue-700 hover:drop-shadow-2xl hover:animate-bounce duration-300">&#9993;</button> */}
                    
                        {/* <button class="float-right ml-2 mr-6 mt-6 bg-blue-400 hover:bg-blue-600 text-white text-sm font-bold rounded px-2 py-1 right-3 ">
                            Reset
                        </button> */}
                    {/* <img src={ RefreshIcon } className='h-6 w-6 z-90 fixed mr-5 mt-5 ' onClick={() => { this.buildObject() }}/> */}
