...
import { CheckBoxWrapper, RadioInput, RadioLabel, RadioMark, WebGLContainerWrapper, Wrapper } from './style'
...
import BGSrc1 from '../../assests/images/bglouoj.jpg';
import $ from 'jquery';
...
class WebGlFemale extends React.Component{
  constructor(props){
    super(props);
    this.state = {
      mobile: false,
...
HideUnHideFab(nameClicked, isShow){
    ...
    // this.visibility['Back'] = !this.visibility['Back'];
    // this.visibility['Churidar'] = !this.visibility['Churidar'];
    // this.visibility['Front'] = !this.visibility['Front'];
    // this.visibility['Length'] = !this.visibility['Length'];
    // this.visibility['Neck'] = !this.visibility['Neck'];
    // this.visibility['Sleeve'] = !this.visibility['Sleeve'];
    const entries = Object.entries(this.visibility);
    //this.visibility['Lady'] = true;
...
HeaderClick(nameClicked){
    this.currentSelection = nameClicked;
    this.tempSelection = nameClicked;
    $('#colors-panel').addClass('on');
...
TileChange(values, Axis){
    if (Axis === 'X'){
        this.TileX = values;
        this.mapOutside[this.fabricId].wrapS = THREE.RepeatWrapping;
        this.mapOutside[this.fabricId].wrapT = THREE.RepeatWrapping;
        this.mapOutside[this.fabricId].repeat.set( this.TileX, this.TileY );    
    }else if (Axis === 'Y'){
        this.TileY = values;
        this.mapOutside[this.fabricId].wrapS = THREE.RepeatWrapping;
        this.mapOutside[this.fabricId].wrapT = THREE.RepeatWrapping;
...
FabricChange(id){
    //this.HeaderClick(this.currentSelection);
    this.currentSelection = this.tempSelection;
    var foundStatus = false;
    let _this = this;
    //this.mapOutside[id] = new THREE.TextureLoader().load( mapsArray[i] );
    this.fabricId = id; 
    if (this.currentSelection === "Dress"){
        this.model.traverse( function( node ) {
            if ( node.name === _this.currentSelection) {
                ...
                node.traverse( function( subNode ) {
                    if ( subNode instanceof THREE.Mesh ) {
                        //console.log('SUBNODES that are meshes: ', subNode);
                        if (subNode.material.length > 1){
                            subNode.material[0].map = _this.mapOutside[id];
                            subNode.material[1].map = _this.mapOutside[id];
                        }else{
                            subNode.material.map = _this.mapOutside[id];
                        }
                        subNode.material.needsUpdate = true;
    ...
    }else if (this.currentSelection === "Churidar"){
        this.model.traverse( function( node ) {
            if ( node.name === _this.currentSelection && node.name !== "Front" && node.name !== "Sleeve") {
                foundStatus = true;
                //console.log('Found node with that name: ', node);
                node.traverse( function( subNode ) {
                    if ( subNode instanceof THREE.Mesh ) {
...
InitModel(){
    var boss = this.scene.children[3];
    for (var i = 0; i<boss.children.length; i++) {
        if (boss.children[i].name === "" || boss.children[i].name === "Dress"){ continue; }
        for (var j = 0; j < boss.children[i].length; j++){
            boss.children[i].children[j].visible = false;
        }
        boss.children[i].visible = false;
    }
}
...
LeftMenuClick(_nameClicked, parentName){
    ...
    var node = this.scene.getObjectByName( bossId );
    var boss = node;
    for (var i = 0; i<boss.children.length; i++) {
        boss.children[i].visible = false;
    }
...
buildObject() {
        ...
        for (var i = 0; i<Object.keys(this.data.menu[m].submenu).length; i++) {
            ...
            if (this.data.menu[m].submenu[i].model) {
                ...
                for (var k = 0; k<Object.keys(this.data.menu[m].submenu[i].model).length; k++) {
                    ...
                    var isVisible = this.data.menu[m].submenu[i].model[k].IsVisible;
                    // order.main_menu[i].sub_elements[j].model.push({name: data.menu[m].submenu[i].submenu[j].model[k].name, texture:'tex1'})

                    this.createMesh(name,path,k,this.mapOutside[1],obj,boss,status, isVisible);
                ...
                for (var j = 0; j<Object.keys(this.data.menu[m].submenu[i].submenu).length; j++) {
                    ...
                    if (this.data.menu[m].submenu[i].submenu[j].model) {
                        ...
                        for (var k = 0; k<Object.keys(this.data.menu[m].submenu[i].submenu[j].model).length; k++) {
                            ...
                            var isVisible = this.data.menu[m].submenu[i].submenu[j].model[k].IsVisible;
                            //var isVisible = this.data.menu[m].submenu[i].subMenu[j].model[k].visible;
                            // order.main_menu[i].sub_elements[j].model.push({name: data.menu[m].submenu[i].submenu[j].model[k].name, texture:'tex1'})

                            this.createMesh(name,path,k,this.mapOutside[1],obj2,boss,status, isVisible);
                        }
                    }
                }
            }
    ...
    // this.visibility['Back'] = true;
    // this.visibility['Churidar'] = true;
    // this.visibility['Front'] = true;
    // this.visibility['Length'] = true;
    // this.visibility['Neck'] = true;
    // this.visibility['Sleeve'] = true;
...
createMesh(name,path,i,map,obj,boss,status,isVisible) {
    let _this = this;
    this.mtlLoader.load( path.slice(0,-4)+".mtl", function( materials ) {
        ...
        _this.objLoader.load(path, function ( object ) {
            var idm = (new THREE.Matrix4()).identity();
            idm.elements[0] = -1;
            idm.elements[10] = 1;
            //object.visible = isVisible;
            object.position.y = -57;
            object.scale.set(0.053, 0.053, 0.053)
            // if (name === "Lady") {
            //     object.position.y = -65;
            //     object.scale.set(0.05, 0.05, 0.05)
            //     //object.applyMatrix4(idm);
            // }else if (name === "Churidar"){
            //     object.position.y = -65;
            //     object.scale.set(0.05, 0.05, 0.05)
            //     // object.position.set(0.7, -65, 0.4);
            //     // object.scale.set(0.050, 0.05, 0.057)
            // }
            // else if (name === "Back"){
            //     object.position.y = -65;
            //     object.scale.set(0.05, 0.05, 0.05)
                
            //     // object.position.set(0.7, -65, 1.0);
            //     // object.scale.set(0.050, 0.05, 0.059)
            // }else if (name === "Length"){
            //     object.position.y = -65;
            //     object.scale.set(0.05, 0.05, 0.05)
            //     // object.position.set(0.7, -65, 1.0);
            //     // object.scale.set(0.050, 0.05, 0.059)
            // }else if (name === "Neck"){
            //     object.position.set(0.7, -65, 4.2)
            //     //object.position.y = -65;
            //     object.scale.set(0.053, 0.05, 0.05)
            //     object.rotateX(-0.03)
            //     object.rotateY(-0.07)
            //     //object.rotateY(0.17)
            //     //object.rotateZ(0.17)
            ...
            if ( object.children.length >= 1 ) {
                ...
                for (var i = 0; i<object.children.length; i++) {
                    ...
                    if (object.children[i].length >= 1){
                        for (var j = 0; j < object.children[i].length; j++){
                            object.children[i].children[j].material.map = map;
                            object.children[i].children[j].material.transparent = true;
                            object.children[i].children[j].material.side = THREE.DoubleSide;
                        }
                    }else {
                        if (object.children[i].material.length > 1 && name !== "Dress"){
                            for (var m = 0; m < object.children[i].material.length; m++){
                                object.children[i].material[m].map = map;
                            }
                            //object.children[i].material.map = map;
                            object.children[i].material.transparent = true;
                            object.children[i].material.side = THREE.DoubleSide;
...
checkCardClick(name){
  ...
  render(){
    return(
    <WebGLContainerWrapper id='webglparent'>
        { !this.props.isAdmin ? (
            <CheckBoxWrapper>
